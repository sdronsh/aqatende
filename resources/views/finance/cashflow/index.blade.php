        ...
        <div class="flex flex-wrap items-center justify-between gap-3 border-b border-gray-200 px-6 py-4">
            ...
            <form method="GET" action="{{ route('finance.cashflow.index') }}" class="flex w-full flex-wrap items-center gap-2">
                ...
                <select class="rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm text-gray-700 shadow-theme-xs" name="order_by">
                    @php $selectedOrderBy = $orderBy ?? request('order_by', 'date_desc'); @endphp
                    <option value="date_desc" @selected($selectedOrderBy === 'date_desc')>Data (mais recente)</option>
                    <option value="date_asc" @selected($selectedOrderBy === 'date_asc')>Data (mais antiga)</option>
