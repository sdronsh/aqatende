    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                ...
                <h2 class="text-lg font-semibold text-gray-800">Editar conta bancaria</h2>
            </div>
        </div>
    </x-slot>

    <form method="POST" action="{{ route('finance.accounts.update', $account) }}" class="rounded-xl border border-gray-200 bg-white p-6 shadow-theme-sm space-y-4">
        @csrf
        @method('PUT')
