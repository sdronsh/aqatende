<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-wrap items-center justify-between gap-3">
            <div>
                ...
                <h2 class="text-lg font-semibold text-gray-800">Especialidades</h2>
            </div>
            <a class="rounded-lg bg-brand-500 px-4 py-2 text-sm font-medium text-white shadow-theme-xs hover:bg-brand-600" href="{{ route('specialties.create') }}">+ Nova</a>
        </div>
    </x-slot>
    ...
    <div class="rounded-xl border border-gray-200 bg-white shadow-theme-sm">
        <div class="flex flex-wrap items-center justify-between gap-3 border-b border-gray-200 px-6 py-4">
            <div class="text-sm font-medium text-gray-700">Lista de Especialidades</div>
            <form method="GET" action="{{ route('specialties.index') }}" class="flex w-full max-w-3xl flex-wrap items-center gap-2">
                ...
                <tbody>
                    @forelse ($specialties as $specialty)
                        <tr class="odd:bg-gray-50">
                            <td class="border border-gray-200 px-4 py-3 font-medium text-gray-800">{{ $specialty->name }}</td>
                            <td class="border border-gray-200 px-4 py-3 text-gray-600">
                                {{ $specialty->active ? 'Ativa' : 'Inativa' }}
                            </td>
                            <td class="border border-gray-200 px-4 py-3">
                                <div class="flex justify-end gap-2">
                                    <a class="rounded-lg border border-brand-500 px-2 py-1 text-xs font-medium text-brand-500 hover:bg-brand-50" href="{{ route('specialties.edit', $specialty) }}">Editar</a>
                                    <form method="POST" action="{{ route('specialties.destroy', $specialty) }}" onsubmit="return confirm('Remover especialidade?');">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="3" class="border border-gray-200 px-6 py-6 text-center text-gray-500">Nenhuma especialidade cadastrada.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
