<x-app-layout>
    ...
    <form method="POST" action="{{ route('security.roles.update', $role) }}" class="rounded-xl border border-gray-200 bg-white p-4 shadow-theme-sm md:p-6">
        @csrf
        @method('PUT')
        @include('security.roles._form', ['role' => $role, 'permissions' => $permissions, 'selected' => $selected])
