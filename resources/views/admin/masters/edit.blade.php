<x-app-layout>
    <x-slot name="header">
        <h2 class="text-lg font-semibold text-gray-800">Editar usuario master</h2>
    </x-slot>

    <form method="POST" action="{{ route('admin.masters.update', $user) }}" class="rounded-xl border border-gray-200 bg-white p-4 shadow-theme-sm md:p-6">
        @csrf
        @method('PUT')
        @include('admin.masters._form', ['user' => $user])
